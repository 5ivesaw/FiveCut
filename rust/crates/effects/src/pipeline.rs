    ...
    pub fn new(context: &GpuContext) -> Self {
        ...
        let shader_sources = [
            ...
            (
                ...
                COLOR_GRADE_SHADER_SOURCE,
                "effects-color-grade",
            ),
            (PIXELATE_SHADER_ID, PIXELATE_SHADER_SOURCE, "effects-pixelate"),
            (SHARPEN_SHADER_ID, SHARPEN_SHADER_SOURCE, "effects-sharpen"),
            (
                FILM_GRAIN_SHADER_ID,
                FILM_GRAIN_SHADER_SOURCE,
                "effects-film-grain",
            ),
        ];
        let pipelines = shader_sources
            .into_iter()
            .map(|(id, source, label)| {
                ...
                let pipeline = create_effect_render_pipeline(
                    context,
                    &pipeline_layout,
...
fn pack_effect_uniforms(
    ...
    let (direction, scalars, supported): ([f32; 2], [f32; 4], &[&str]) = match shader {
        GAUSSIAN_BLUR_SHADER_ID => (
            read_vec2_uniform(pass, "u_direction")?,
            [
        ...
        PIXELATE_SHADER_ID => (
            [0.0, 0.0],
            [read_number_uniform(pass, "u_pixel_size")?, 0.0, 0.0, 0.0],
            &["u_pixel_size"],
        ),
        SHARPEN_SHADER_ID => (
            [0.0, 0.0],
            [read_number_uniform(pass, "u_amount")?, 0.0, 0.0, 0.0],
            &["u_amount"],
        ),
        FILM_GRAIN_SHADER_ID => (
            [0.0, 0.0],
            [
                ...
                read_number_uniform(pass, "u_size")?,
                0.0,
                0.0,
            ],
            &["u_intensity", "u_size"],
    ...
    ensure_supported_uniforms(pass, supported)?;
