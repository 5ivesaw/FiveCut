...
export const useSoundsStore = create<SoundsStore>((set, get) => ({
	...
	addSoundToTimeline: async ({ sound }) => {
		...
		try {
			...
			const extension =
				blob.type.split("/")[1]?.replace("mpeg", "mp3") ||
				sound.type ||
				"audio";
			const safeName =
				sound.name.replaceAll(/[^a-zA-Z0-9._ -]+/g, "").trim() || "sound";
			const file = new File([blob], `${safeName}.${extension}`, {
				type: blob.type || "audio/mpeg",
			});
			const [processed] = await processMediaAssets({ files: [file] });
			if (!processed) {
				throw new Error("Could not decode the downloaded sound");
			}
			...
			if (!asset) {
				throw new Error("Could not save the sound in this project");
			}

			const duration = Math.max(
				1,
				Math.round((asset.duration || sound.duration) * TICKS_PER_SECOND),
			);
			...
			toast.success("Sound downloaded and added", {
				description: `${sound.name} is cached with this project for offline use.`,
			});
			return true;
		} catch (error) {
