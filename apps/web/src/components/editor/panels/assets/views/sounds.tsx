...
function MusicView() {
	...
	useEffect(() => {
		...
		const timer = window.setTimeout(async () => {
			...
			try {
				...
				if (!response.ok) {
					throw new Error(`Music search returned HTTP ${response.status}`);
				}
	...
	return (
		<div className="mt-1 flex h-full flex-col gap-4">
			<Input
				placeholder="Search openly licensed music"
				value={query}
			...
			<p className="text-muted-foreground text-[0.67rem] leading-relaxed">
				Tracks come from Openverse. Adding one downloads and caches it in your
				project; verify its displayed license before publishing.
			</p>
			<ScrollArea className="h-full flex-1">
				<div className="flex flex-col gap-4">
					{isLoading ? (
						...
						<div className="text-muted-foreground text-sm">
							No music found. The built-in sound effects remain available
							offline.
						</div>
					) : (
						results.map((sound) => (
